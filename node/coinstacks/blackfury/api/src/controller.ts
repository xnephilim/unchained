...
import { predeploys, getContractInterface } from '@eth-blackfury/contracts-bedrock'
...
import { BlackfuryGasEstimate, BlackfuryGasFees } from './models'
...
export const gasOracle = new GasOracle({ logger, provider, coinstack: 'blackfury' })
...
export class Blackfury extends Controller implements BaseAPI, API {
   ...
   *
   * @returns {Promise<BlackfuryGasEstimate>} estimated gas cost
   *
   * @example data "0x"
   ...
   * @example value "1337"
   */
  @Example<BlackfuryGasEstimate>({ gasLimit: '21000', l1GasLimit: '3800' })
  @Response<ValidationError>(422, 'Validation Error')
  @Response<InternalServerError>(500, 'Internal Server Error')
  @Get('/gas/estimate')
  async estimateGas(
   ...
   *
   * @returns {Promise<BlackfuryGasFees>} current fees specified in wei
   */
  @Example<BlackfuryGasFees>({
  ...
  @Response<InternalServerError>(500, 'Internal Server Error')
  @Get('/gas/fees')
  async getGasFees(): Promise<BlackfuryGasFees> {
  ...
  @Example<TokenMetadata>({
    ...
    description: '',
    media: {
      url: 'https://dftqodalckck7.cloudfront.net/blackfury/15976.svg',
      type: 'image',
    },
  })
  @Response<ValidationError>(422, 'Validation Error')
